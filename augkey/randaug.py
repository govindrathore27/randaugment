...
class RandAugment:
    ...
    def __init__(self, N=2, operations=None, magnitude_policy=None):
        ...
        if operations is None:
            ...
            self.operations = dict(enumerate([
                ops.Identity(),
                ops.AutoContrast(),
                ops.Equalize(),
                ops.Rotate(np.linspace(0, math.radians(30), 30)),
                ops.Solarize(np.linspace(0, 256, 30, dtype=int)),
                ops.Posterize(np.linspace(4, 8, 30, dtype=int)),
                ops.Color(np.linspace(0, 0.9, 30)),
                ops.Contrast(np.linspace(0, 0.9, 30)),
                ops.Brightness(np.linspace(0, 0.9, 30)),
                ops.Sharpness(np.linspace(0, 0.9, 30)),
                ops.ShearX(np.linspace(0, 0.3, 30)),
                ops.ShearY(np.linspace(0, 0.3, 30)),
                ops.TranslateX(np.linspace(0, 150, 30)),
                ops.TranslateY(np.linspace(0, 150, 30))
            ]))
        else:
    ...
    def plan_augment(self):
        ...
        # The augmentation is a list of operations
        aug_ops = np.random.randint(
            len(self.operations),
    ...
    def apply_image(self, image, magnitude, operation_indices, directions):
        ...
        image : PIL.Image
        """
        for direction, operation_index in zip(directions, operation_indices):
            operation = self.operations[operation_index]
            image = operation.transform_image(image, magnitude, direction)
        return image

    def apply_keypoints(self, keypoints, magnitude, operation_indices,
                        directions):
        ...
        for direction, operation_index in zip(directions, operation_indices):
            ...
            keypoints = operation.transform_keypoints(
                keypoints, magnitude, direction)
        return keypoints

    def apply_inv_keypoints(self, keypoints, magnitude, operation_indices,
                            directions):
